...
void main() {
  ...
  test('Scale tests', () {
    ...
    expect(Scale.major.inKey(MajorKey.C).toString(), '[C, D, E, F, G, A, B, C]');
    expect(Scale.ionian.inKey(MajorKey.C).toString(), '[C, D, E, F, G, A, B, C]');
    expect(Scale.dorian.inKey(MajorKey.D).toString(), '[D, E, F, G, A, B, C, D]');
    expect(Scale.phrygian.inKey(MajorKey.E).toString(), '[E, F, G, A, B, C, D, E]');
    expect(Scale.lydian.inKey(MajorKey.F).toString(), '[F, G, A, B, C, D, E, F]');
    expect(Scale.mixolydian.inKey(MajorKey.G).toString(), '[G, A, B, C, D, E, F, G]');
    expect(Scale.aeolian.inKey(MajorKey.A).toString(), '[A, B, C, D, E, F, G, A]');
    expect(Scale.minor.inKey(MajorKey.A).toString(), '[A, B, C, D, E, F, G, A]');
    expect(Scale.locrian.inKey(MajorKey.B).toString(), '[B, C, D, E, F, G, A, B]');
    /*
    ...
    expect(Scale.majorPentatonic.inKey(MajorKey.C).toString(), '[C, D, E, G, A, C]');
    expect(Scale.minorPentatonic.inKey(MajorKey.A).toString(), '[A, C, D, E, G, A]');
    expect(Scale.minorPentatonic.inKey(MajorKey.C).toString(), '[C, E♭, F, G, B♭, C]');
  });
}
