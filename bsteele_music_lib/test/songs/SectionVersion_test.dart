  ...
  test('parseString', () {
    for (Section section in Section.values) {
      for (int i = 0; i < 10; i++) {
        ...
        sectionVersion = SectionVersion.parseString(section.abbreviation.toString() + ':  ');
        expect(sectionVersionExpected, sectionVersion);
  ...
  test('parseInContext', () {
    for (Section section in Section.values) {
      ...
      for (int i = 0; i < 10; i++) {
        ...
        MarkedString markedString = MarkedString(section.toString() + (i > 0 ? i.toString() : '') + ':' + chords);
        ...
        chords = chords.trim();
        markedString = MarkedString(section.toString() + (i > 0 ? i.toString() : '') + ':' + chords);
        sectionVersion = SectionVersion.parse(markedString);
