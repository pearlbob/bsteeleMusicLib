...
String chordComponentScaleNotesToString(Key key, ScaleChord scaleChord) {
  ...
  for (ChordComponent chordComponent in chordComponents) {
    if (chordComponent != chordComponents.first) sb.write(" ");
    sb.write(key
        .getScaleNoteByHalfStep(key.getHalfStep() + chordComponent.halfSteps));
  }
  return sb.toString();
}
...
void main() {
  ...
  test("testScaleChordParse", () {
    ...
    ScaleChord ref =
        ScaleChord(ScaleNote.get(ScaleNoteEnum.A), ChordDescriptor.dominant13);
    expect(a, ref);
    expect(
        ScaleChord.parseString("A13"),
        new ScaleChord(
            ScaleNote.get(ScaleNoteEnum.A), ChordDescriptor.dominant13));
    expect(
        new ScaleChord(ScaleNote.get(ScaleNoteEnum.F), ChordDescriptor.major),
        ScaleChord.parseString("F"));
    expect(
        new ScaleChord(ScaleNote.get(ScaleNoteEnum.F), ChordDescriptor.major),
        ScaleChord.parseString("FGm"));
    expect(
        new ScaleChord(ScaleNote.get(ScaleNoteEnum.F), ChordDescriptor.minor),
        ScaleChord.parseString("Fm"));
    expect(
        new ScaleChord(ScaleNote.get(ScaleNoteEnum.Fs), ChordDescriptor.minor),
        ScaleChord.parseString("F#m"));
    expect(
        new ScaleChord(ScaleNote.get(ScaleNoteEnum.Fs), ChordDescriptor.minor),
        ScaleChord.parseString("F#mGm"));
    expect(
        new ScaleChord(
            ScaleNote.get(ScaleNoteEnum.D), ChordDescriptor.diminished),
        ScaleChord.parseString("Ddim/G"));
    expect(
        new ScaleChord(
            ScaleNote.get(ScaleNoteEnum.A), ChordDescriptor.diminished),
        ScaleChord.parseString("Adim/G"));
    expect(
        new ScaleChord(ScaleNote.get(ScaleNoteEnum.X), ChordDescriptor.major),
        ScaleChord.parseString("X/G"));
  });
}
